...
export const BottomNav: React.FC<BottomNavProps> = ({ currentView, setView }) => {
  ...
  const containerClass = useMemo(() => 
    `relative pointer-events-auto flex items-center gap-1.5 sm:gap-2 p-1.5 sm:p-2 rounded-full shadow-2xl transition-colors duration-500 overflow-hidden ${
      isDarkMode 
        ? 'bg-[#0F0F11] border border-white/5 shadow-black/60' 
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <div 
                className={`overflow-hidden flex items-center whitespace-nowrap transition-all duration-500 ${
                  isActive ? 'w-auto opacity-100 ml-2 sm:ml-3' : 'w-0 opacity-0 ml-0'
                }`}
              >
