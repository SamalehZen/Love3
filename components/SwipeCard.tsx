...
import { swipeConfig } from '../constants/theme';
...
export const SwipeCard: React.FC<SwipeCardProps> = ({ 
  ...
  const { threshold: SWIPE_THRESHOLD, rotationFactor: ROTATION_FACTOR, exitVelocity: EXIT_VELOCITY } = swipeConfig;
  ...
  const handleDragEnd = useCallback(() => {
    ...
    if (Math.abs(dragX) > SWIPE_THRESHOLD) {
      ...
      const velocity = dragX > 0 ? EXIT_VELOCITY : -EXIT_VELOCITY;
      setExitVelocity(velocity);
      trigger(direction === 'like' ? 'success' : 'light');
  ...
  }, [isDragging, dragX, SWIPE_THRESHOLD, EXIT_VELOCITY, onAction, trigger]);
  ...
  const handleButtonAction = useCallback((action: 'like' | 'reject' | 'super') => {
    trigger(action === 'like' || action === 'super' ? 'success' : 'light');
    if (action === 'super') {
      onAction(action);
      return;
    }
    setExitVelocity(action === 'like' ? EXIT_VELOCITY : -EXIT_VELOCITY);
    setTimeout(() => onAction(action), 200);
  }, [onAction, trigger, EXIT_VELOCITY]);
  ...
  const backCardStyle = useMemo(() => ({
    ...
    backgroundColor: isDarkMode ? '#1C1C1E' : '#FFFFFF'
  }), [backCardScale, backCardY, backCardOpacity, isDragging, isDarkMode]);
      ...
      <div className="flex justify-between items-center mb-4 px-2 z-50">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-full overflow-hidden p-0.5 border ${isDarkMode ? 'border-white/20' : 'border-gray-200'}`}>
            <img src="https://picsum.photos/100/100?random=1" className="w-full h-full rounded-full object-cover" alt="User" loading="lazy" />
          </div>
          <div>
